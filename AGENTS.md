...
- `.vercelignore` is the line between "in the repo" and "on the internet". The repo tree IS the site, so every tracked file is served at `https://isaacperez.co/<path>` unless it is listed there: `AGENTS.md`, `CLAUDE.md`, `docs/`, `Resume.pdf`, the retired game bundle (`js/game.js`, `css/game.css`, `css/rpg.css`) and `images/` are repo-only. Add any new doc, note or unpublished asset to it in the same commit that adds the file, and never assume "nothing links it" makes a path private — `/AGENTS.md` and `/Resume.pdf` are exactly what a directory scanner guesses. (added)
